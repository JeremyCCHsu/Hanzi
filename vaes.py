...
class VariationalAutoencoder(object):
    ...
    def partial_fit(self, X):
        ...
        """
        opt, rc, lc = self.sess.run(
            (self.optimizer, self.reconst_loss, self.latent_loss), 
            feed_dict={self.x: X})
        return rc, lc
...
def mask_imgs(mask, imgs):
    n = szImg // 2
    if mask == 'left':
        mask = np.concatenate(
            [np.ones((szImg, n)), np.zeros((szImg, szImg-n))],
            axis=1)
    if mask == 'right':
        mask = np.concatenate(
            [np.zeros((szImg, szImg-n)), np.ones((szImg, n))],
            axis=1)
    if mask == 'top':
        mask = np.concatenate(
            [np.ones((n, szImg)), np.zeros((szImg- n, szImg))],
            axis=0)
    if mask == 'bot':
        mask = np.concatenate(
            [np.zeros((szImg-n, szImg)), np.ones((n, szImg))],
            axis=0)
    imgs_cut = map(lambda x: x*mask, imgs)
    imgs_cut = np.asarray(imgs_cut)
    imgs_cut = np.reshape(imgs_cut, (-1, imgs_cut.shape[1]*imgs_cut.shape[2]))
    return imgs_cut
...
def draw(vae_2d, img_test, batch_size, szImg=50, grid=12):
    # szImg = 50
    # n = 12
    # img_test = imgs[:128, :]
...
def train(imgs, network_architecture, learning_rate=0.001,
    ...
    for epoch in range(training_epochs):
        ...
        for batch_xs in batches:
            ...
            rc, lc = vae.partial_fit(batch_xs)
...
vaes = list()
for mask in ['left', 'right', 'top', 'bot']:
    img_part = mask_imgs(mask, imgs)
    vae_2d = train(img_part,
        network_architecture, 
        batch_size=128,
        learning_rate=1e-4,
        training_epochs=50)
